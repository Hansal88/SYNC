    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-left">
      <div className={`w-full transition-all duration-500 max-w-md`}>
        <div className="flex justify-center gap-2 mb-8">
          {[1].map((i) => (
            <div key={i} className={`h-1 w-12 rounded-full transition-all bg-blue-600 animate-slideInDown`} />
          ))}
        </div>

        <div className="bg-slate-900 border border-slate-800 p-8 md:p-10 rounded-[2.5rem] shadow-2xl animate-slideInUp">
          {step === 1 && (
            <form onSubmit={handleInitialSignup} className="space-y-5">
              <div className="text-center mb-8">
                <h2 className="text-3xl font-black text-white uppercase tracking-tighter animate-slideInDown">Create Account</h2>
                <p className="text-slate-500 mt-2 animate-slideInDown animation-delay-100">Join the skill exchange community</p>
              </div>
              <div className="space-y-4">
                <div className="relative animate-slideInUp animation-delay-200">
                  ...
                  <input 
                    ...
                    required 
                    className="signup-input focus:ring-2 focus:ring-blue-500/50 hover:border-slate-700 transition-all duration-300" 
                ...
                <div className="relative animate-slideInUp animation-delay-400">
                ...
                <div className="relative animate-slideInUp animation-delay-500">
                  ...
                  <button 
                    ...
                    className="absolute right-4 top-4 text-slate-500 hover:text-white transition-colors duration-300"
                  >
                ...
                <div className="p-3 bg-blue-600/10 border border-blue-600/30 rounded-xl animate-slideInUp animation-delay-700 hover:bg-blue-600/20 transition-all duration-300">
              ...
              <button 
                ...
                disabled={loading}
                className={`w-full font-bold py-4 rounded-2xl transition-all flex items-center justify-center gap-2 group animate-slideInUp animation-delay-800 ${
                  loading 
                    ? 'bg-slate-700 text-slate-400 cursor-not-allowed shadow-none' 
                    : 'bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 hover:scale-105'
                }`}
              >
                {loading ? (
                  <>
                  ...
                  <>
                    Send Verification Code <ChevronRight size={20} className="group-hover:translate-x-1 transition-transform duration-300" />
                  </>
                )}
              </button>
            </form>
        ...
        <p className="text-center text-slate-500 mt-8 animate-slideInUp animation-delay-900">
          Already have an account? <Link to="/login" className="text-blue-500 font-bold hover:text-blue-400 hover:underline transition-colors duration-300">Sign In</Link>
        </p>
      </div>
    </div>
  );
